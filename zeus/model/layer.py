...
import torch
from torch.nn.modules.transformer import (_get_activation_fn, Module, Tensor,
...
class TransformerEncoderLayer(Module):
    ...
    def forward(self, src: Tensor, src_mask: Optional[Tensor] = None,
        ...
        if isinstance(src_mask, tuple):
            ...
            attn = partial(checkpoint, self.self_attn, use_reentrant=True) if self.recompute_attn else self.self_attn

            global_tokens_src2 = attn(
                global_tokens_src, global_and_train_tokens_src,
                global_and_train_tokens_src, None, True, global_src_mask
            )[0]
            train_tokens_src2 = attn(
                train_tokens_src, global_tokens_src, global_tokens_src, None, True, trainset_src_mask
            )[0]
